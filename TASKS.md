...
- [x] `constants.py` — add `PROFILES` ordered dict and `PROFILE_NAMES` list:
  ```python
  PROFILES = {
...
- [x] `state.py` — remove `m8_var`; add `profile_var = None` (will be `tk.StringVar`, default `"Generic"`)

- [x] `operations.py` — rename `_m8_truncate(new_name, dest_path_str)` →
...
- [x] `preview.py` — update import from `_m8_truncate` to `_apply_path_limit`; update call site to pass `limit` from active profile

- [x] `builders.py` — in `build_center()`: remove "M8 Friendly" `ttk.Checkbutton`;
...
- [x] `builders.py` — in `build_app()`: remove `state.m8_var` trace; add trace on
  `state.profile_var` that calls `preview.refresh_preview()`

- [x] `builders.py` — in `toggle_theme()`: save/restore `profile_var` alongside the other vars
...
- [x] `operations.py` — add `_compute_output()` immediately below `_apply_path_limit()`:
...
- [x] `preview.py` — import `_compute_output`; replace the inline rename logic in
...
- [x] `state.py` — add `struct_mode_var = None` (will be `tk.StringVar`, default `"flat"`)

- [x] `builders.py` — in `build_center()`, add below the profile combobox:
  ```
...
- [x] `builders.py` — in `build_app()`: add trace on `state.struct_mode_var` → `preview.refresh_preview()`

- [x] `builders.py` — in `toggle_theme()`: save/restore `struct_mode_var`

- [x] `builders.py` — in `build_deck_b()`: add a third `"Subfolder"` column to `state.preview_tree`
  (header text `"Subfolder"`, initial `width=0` so it is hidden in flat mode)

- [x] `preview.py` — in `refresh_preview()`: after calling `_compute_output()`, set the
...
- [x] `operations.py` — `run_tool()`: read `state.struct_mode_var` and resolve
  `path_limit = constants.PROFILES[state.profile_var.get()]["path_limit"]`;
  pass both to the worker thread

- [x] `operations.py` — `_run_worker()` signature: add `struct_mode` and `path_limit` params

- [x] `operations.py` — inside the file loop, replace the current inline rename logic with:
  ```python
...
- [x] `operations.py` — update log message to include subfolder when non-empty:
  ```python
...
- [x] `builders.py` — `build_status_bar()`: update version label `"v0.10"` → `"v0.11"`
- [x] *(Optional)* Update subtitle in `build_header()` from `"Audio Sample Organiser"`
