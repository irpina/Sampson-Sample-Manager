...
def build_header(parent):
    frame = tk.Frame(parent, bg=theme.BG_SURF2, height=52)
...
def build_deck_a(parent):
    ...
    tk.Frame(strip, bg=theme.CYAN, width=3).pack(side="left", fill="y")
...
def build_center(parent):
    ...
    state.run_btn.grid(row=5, column=0, padx=16, sticky="ew")

    ttk.Button(frame, text="Clear log", style="Outlined.TButton",
               command=log_panel.clear_log).grid(row=6, column=0, padx=16, pady=(10, 16))
...
def build_deck_b(parent):
    ...
    tk.Frame(strip, bg=theme.AMBER, width=3).pack(side="left", fill="y")
    ...
    state.preview_tree.column("original", width=160, anchor="w", minwidth=80)
    state.preview_tree.column("renamed",  width=200, anchor="w", minwidth=80)
...
def build_status_bar(parent):
    frame = tk.Frame(parent, bg=theme.BG_SURF2, height=36)
    frame.pack_propagate(False)
